...
/** TODO: support small_matrix_value option so this is configurable */
...
export function toMPSFormat(input: MPSFormatInput): string {
  ...
  for (const coeffs of varCoeffs.values()) {
    for (const [rowName, coeff] of coeffs) {
      if (rowName !== "obj" && Math.abs(coeff) <= EPS) {
        coeffs.delete(rowName);
      }
    }
  }
